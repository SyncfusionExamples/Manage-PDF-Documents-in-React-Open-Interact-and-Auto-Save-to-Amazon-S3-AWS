...
namespace Syncfusion.EJ2.FileManager.AmazonS3DocumentManager
{
    public class AmazonS3DocumentManager
    {
        ...
        public void GetBucketList()
        {
            ...
            var s3Objects = response.S3Objects.Where(x => x.Key.Split(".").Length != 2);

            if (s3Objects.Any())
            {
                RootName = s3Objects.First().Key;
                RootName = RootName.Replace("../", "");
            }
            else
            {
                // Handle the case where no objects are found
                RootName = string.Empty; // or any default value you prefer
            }
        }
        ...
        public FileManagerResponse GetFiles(string path, bool showHiddenItems, params FileManagerDirectoryContent[] data)
        {
            ...
            try
            {
                ...
                {
                    string adjustedPath = this.RootName.Replace("/", "") + path;
                    ListingObjectsAsync("/", adjustedPath, false).Wait();
                }
                ...
                if (response.S3Objects.Count > 0)
                {
                    filesS3 = response.S3Objects
                        ...
                        .Select(y =>
                        {
                            ...
                            string basePath = RootName.Replace("/", "") + path;

                            // Ensure the basePath is not longer than the key
                            if (key.StartsWith(basePath))
                            {
                                string relativeKey = key.Substring(basePath.Length).Replace("/", "");
                                string extension = Path.GetExtension(key);

                                return CreateDirectoryContentInstance(
                                    relativeKey,
                                    true,
                                    extension,
                                    y.Size,
                                    ...
                                    y.LastModified,
                                    this.checkChild(key),
                                    getFilterPath(key, path)
                                );
                            }
                            else
                            {
                                // Handle the case where the key does not start with basePath
                                return null;
                            }
                        })
